{% macro filtrer_ventes_immobilieres(source_dvf) %}
    SELECT 
        ...
        -- read_csv_auto types an all-numeric dvf code_commune as INT; cast to text so the join to
        -- infos_communes.code_commune (VARCHAR, includes Corsica codes like '2A008') doesn't coerce.
        CAST(code_commune AS TEXT) as code_commune
    FROM
        source_dvf 
     WHERE 
        EXISTS (
