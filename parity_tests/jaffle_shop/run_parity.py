...
ORACLE_SCHEMA = "main"
DUCKRUN_WH = Path("C:/tmp/js_wh") if os.name == "nt" else Path("/tmp/js_wh")
DUCKRUN_SCHEMA = "main"
...
def build_duckrun():
    """dbt build on the duckrun profile here → a fresh Delta warehouse."""
    import shutil
    shutil.rmtree(DUCKRUN_WH, ignore_errors=True)
    env = {**os.environ,
           "DBT_PROFILES_DIR": str(HERE),
           "WAREHOUSE_PATH": str(DUCKRUN_WH),
...
def diff() -> bool:
    ...
    all_ok = True
    for t in tabs:
        dpath = DUCKRUN_WH / DUCKRUN_SCHEMA / t
        if not dpath.is_dir():
        ...
        d_sel = f"select {sel} from delta_scan('{dpath.as_posix()}')"
        no = c.execute(f'select count(*) from o.{ORACLE_SCHEMA}."{t}"').fetchone()[0]
        nd = c.execute(f"select count(*) from delta_scan('{dpath.as_posix()}')").fetchone()[0]
