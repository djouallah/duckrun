...
{#-- Upstream created a PostGIS GIST index on geopoint via a post_hook. duckrun materializes the model
    as a Delta table (geopoint stored as WKB BLOB) and surfaces it back as a delta_scan view, so a
    DuckDB index post_hook is neither possible nor meaningful here; it is dropped. --#}
