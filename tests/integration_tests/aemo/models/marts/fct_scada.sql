...
{{ config(
    ...
    pre_hook=[
      "SET VARIABLE scada_daily_paths = (SELECT COALESCE(NULLIF(list('{{ get_csv_archive_path() }}' || archive_path), []), ['']) FROM (SELECT archive_path FROM {{ ref('stg_csv_archive_log') }} WHERE source_type = 'daily'{% if is_incremental() %} AND csv_filename NOT IN (SELECT DISTINCT daily_file FROM {{ this }} WHERE daily_file IS NOT NULL){% endif %} ORDER BY source_filename LIMIT {{ env_var('process_limit', '25') }}))",
      "SET VARIABLE scada_today_paths = (SELECT COALESCE(NULLIF(list('{{ get_csv_archive_path() }}' || archive_path), []), ['']) FROM (SELECT archive_path FROM {{ ref('stg_csv_archive_log') }} WHERE source_type = 'scada_today'{% if is_incremental() %} AND csv_filename NOT IN (SELECT DISTINCT intraday_file FROM {{ this }} WHERE intraday_file IS NOT NULL){% endif %} ORDER BY source_filename LIMIT {{ env_var('process_limit', '25') }}))"
    ]
) }}
...
{#-- Gate each feed independently: the guard used to sum both counts, so a run with only one feed
    backlogged (e.g. daily = 0, scada_today > 0) still entered the body and then hit
...
SELECT COUNT(*) AS cnt FROM {{ ref('stg_csv_archive_log') }} WHERE source_type = 'scada_today'
