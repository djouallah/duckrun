"""duckrun get_stats for the Contoso Sales layout copies → run_report.json (tables.<t>.parquet):
per-table summary, row_group_detail, and the raw per-column-chunk parquet_metadata (encodings,
dictionary, sizes, stats) under column_chunks. One file, everything in it. Mirror of the AEMO
benchmark's table_stats.py, retargeted to the `sales_*` layout copies.
...
def main():
    ...
    print("\n=== Per-table summary — get_stats('sales*') ===")
    try:
    ...
    det = con.get_stats("sales*", detailed=True)
