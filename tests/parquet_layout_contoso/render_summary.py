...
wall-clock and Parquet metadata only. Medians only — never a mean; ties render as `=`. Mirror of the
AEMO benchmark's render_summary.py; the Contoso column vocabulary comes from the local render_report.
...
def lbl(model):
    return rr._short(model)  # layout label: physical name minus its contoso_/sales_ prefix


def _noisy_cols(rep, thresh=25.0):
    """Probe columns whose cold spread exceeds `thresh`% in ANY layout — too noisy to quote."""
    tim = rep.get("timings", {})
    noisy = set()
    for m in tim:
...
def s1_header(rep):
    ...
    w("# Specialist findings — Contoso Direct Lake layout benchmark")
    w()
    ...
    w()
    w(f"{len(rep.get('tables', {}))} layouts of the same {rows/1e6:,.1f}M-row Sales fact; DAX over "
      f"XMLA against Direct Lake; {inp.get('cold_repeats')} cold cycles (dehydrate→query) per query, "
      f"medians reported.")
    w()
...
def s7_caveats(rep, analysis):
    ...
    w("- single dataset and shape (Contoso Sales: numeric fact columns, integer surrogate keys, one "
      "date key); conclusions do not generalize to high-cardinality-string or wide-dimension tables.")
    w()
...
def verify_verdicts(rep, analysis):
    """Orientation guard: the verdict winner must agree with the per-query cold-median majority over
    the SAME queries the verdict aggregates — a disagreement there is a true ratio inversion and is
    fatal. The summed marginal PROBE cost is a second view over a DIFFERENT (probe-only) subset, so a
    disagreement there is a non-fatal note. Returns (errors, notes)."""
    tim = rep.get("timings", {})
    cc = analysis.get("cold_column_cost", {})
