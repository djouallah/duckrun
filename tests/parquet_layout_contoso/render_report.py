"""Render run_report.json to the GitHub job summary AND compute the derived `analysis` block, which
is merged back into the same file. Pure post-processing — everything here is recomputable offline
from the raw values already in run_report.json. Layout in, wall-clock out; no engine internals.
Mirror of the AEMO benchmark's render_report.py, retargeted to the Contoso Sales fact vocabulary.
...
# reaches the fact OrderDate; brand filter reaches the fact via ProductKey).
_LADDER_COLS = {"sel_1yr": ["OrderDate"], "sel_1mo": ["OrderDate"],
                "sel_1brand": ["ProductKey"], "sel_1brand_1mo": ["ProductKey", "OrderDate"]}
...
def _table(model):
    """Physical table name for a semantic-model name: sales + the model's suffix
    (contoso_auto_sort → sales_auto_sort)."""
    return "sales" + model.removeprefix("contoso")


def _short(model):
    """Layout label: the physical name minus its contoso_/sales_ prefix
    (contoso_auto_sort → auto_sort, sales_vorder → vorder)."""
    return model.removeprefix("contoso_").removeprefix("sales_") or model
...
def _bounds(chunk):
    ...
    (string columns have ONLY these); fall back to legacy stats_min/stats_max otherwise."""
...
def _agg_verdict(base_t, model_t, key, spread_key):
    """Aggregate base-vs-model verdict for one metric, tie rule applied per query."""
    bt = mt = 0.0
    per = []
...
def compute_analysis(rep):
    ...
    # skipping: pair each ladder query's first_touch_ms with the clustering of its filtered column(s).
...
def _layout_table(rep, analysis):
    ...
    column (tie rule). Assumes a single challenger (auto_sort vs vorder)."""
    tables = rep.get("tables", {})
...
def main():
    ...
    run = rep.get("run", {})
    _write(f"# Contoso Direct Lake query benchmark — `{run.get('sha')}` "
