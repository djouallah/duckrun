...
def _compression(chunks):
    ...
    dictionary_page_offset, summed over row groups), physical parquet type, and whether ANY chunk
    carried a dictionary page — the last two catch the arrow-rs FLBA gap (a DECIMAL with precision
    > 18 is written PLAIN, no dictionary)."""
    out = {}
    for c in chunks:
