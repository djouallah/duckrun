...
| [`connection_api/`](connection_api/) | Tests for the top-level **`duckrun.connect()`** notebook API. `test_local_filesystem.py` = offline regression suite; `test_method_matrix.py` = one discrete test per method, rendered into a scorecard. | `cores.yml` (`adapter` job — every push/PR + release gate; `connection-card` job → method scorecard card) |
