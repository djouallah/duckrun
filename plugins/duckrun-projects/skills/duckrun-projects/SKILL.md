...
| `max_row_group_size` | explicit parquet row-group ceiling in **rows** for this model's writes (deltalake's spelling). Bypasses the adaptive planner-estimate sizing — use it when a big model's *first* build must land full 16M-row Direct Lake segments (e.g. `16000000`). Preserved by post-write compaction; the delta_rs merge write itself keeps defaults, post-merge compaction folds files into this geometry |
