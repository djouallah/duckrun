...
| `write.mode("append_if_unchanged")` (alias `safeappend`) | fail-loud compare-and-swap append (commits only if the table version hasn't moved) | no Spark `SaveMode` for it — it's the duckrun/dbt concurrency primitive |
