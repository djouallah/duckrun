...
Note that this loop grounds the **values** in the table above; it does not ground *how the row count reaching them is obtained*. Changes to the estimate itself — the prior-version log floor, the 6M guess floor — do not move any property here, they only make the existing rule reach the size it always intended. They are still run through the benchmark, because a more accurate estimate does change realized geometry on real tables.
