...
| **Row group size** | adaptive · 1M–16M rows | **A max, not a constant.** A full-table write (overwrite) and post-write compaction size each row group from the result's row count — `ceil(rows / DUCKRUN_RG_LANES)`, default 8 lanes, floored at **1M** and capped at **16M** rows — so a small table lands ~8 segments (transcoding on several lanes) instead of one or two giant ones; a table of ≥128M rows (8×16M) uses the flat 16M ceiling. Appends are not sized (delta_rs defaults; folded in by later compaction). The cap exists because one Parquet row group maps to one column segment in a Direct Lake reader, and Fabric prefers segments in the **8–16M** row band. 16M sits at the top of that band (kept under 2²⁴ so one row group still maps to one segment); arrow-rs buffers a full uncompressed row group per open writer, so this is also the write-memory ceiling. arrow-rs's ~1M default would produce thousands of small segments. Staying under 2²⁴ is a **safe default, not the dominant term**: exceeding it degrades gracefully (a row group simply spans more than one segment), and that penalty is readily outweighed by a good **sort key**, which is the larger lever on read performance (see [Automatic sorting](#automatic-sorting)). Order of priority is sort key first, row-group size second. |
...
The **incremental writes — merge and append — take the compromise one step further and opt out entirely**: they write with none of these properties (delta_rs defaults), so an incremental write stays lean and never rewrites large files or materializes large dictionaries to touch a few rows. They are transient: threshold-gated compaction folds those loose files back into this layout on a later pass. Only full-table writes (overwrite / replace-where) and compaction produce the read layout directly — which is why the adaptive small-table sizing above lives on those two paths, not the append path (compaction gets the row count for free from the log; the overwrite path estimates it from the planner).
...
One caveat dominates everything here, and it is the reason this whole page is framed as heuristics: **this was tuned against a single dataset** — one ~140M-row AEMO fact table. That is one workload, not a representative sample. The numbers are grounded on that one benchmark; a table with very different width, cardinality, or skew may well have its sweet spot elsewhere. The comparison is reproducible — see `tests/parquet_layout_tests/` and the manual, non-gating `parquet_layout.yml` workflow, which builds `fct_summary_auto_sort` (duckrun) against `fct_summary_vorder` (Spark V-Order) and runs the DAX comparison in `xmla_compare.py` — so the honest way to trust these defaults for *your* data is to run the same comparison on it.
