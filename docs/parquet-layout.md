...
target file size = rows per group × modelled bytes/row  # clamped to [8 MB, 128 MB]
...
There is nothing left that differs between them: overwrite, append, and a **`replaceWhere`** write (the microbatch strategy, `INSERT … REPLACE WHERE`) all keep the same fixed 8M ceiling and let the 128 MB file roll pick the actual group.
