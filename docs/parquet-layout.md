...
- **Adaptive row groups (1M–16M rows)** — a row group becomes a segment as-is, so row-group sizing *is* segment sizing; anything from **1M to 16M rows is a fine segment (16M is the ceiling)**, a property of the in-memory engine, not of Parquet. One refinement: a **small** table is written with smaller row groups targeting ~8 groups, so it still transcodes on several lanes instead of one or two; tables of ≥128M rows keep the 16M profile. This applies to both paths that produce the read layout: a full-table **overwrite** (`CREATE TABLE … AS SELECT`, a dbt `table` model, `--full-refresh` — sized from the planner's row estimate, raised to the *existing* table's exact log row count when that is larger, and otherwise floored at **8M** because that estimate is only a guess) and **compaction** (sized from the live row count, which is free in the Delta log — no estimate needed, so it keeps the finer **1M** floor). (`DUCKRUN_RG_LANES`, default 8; overwrite is computed at one seam so the SQL and dbt paths behave identically.)
...
| **Row group size** | adaptive · 1M–16M rows | **A max, not a constant.** A full-table write (overwrite) and post-write compaction size each row group from the result's row count — `ceil(rows / DUCKRUN_RG_LANES)`, default 8 lanes, capped at **16M** rows — so a small table lands several segments (transcoding on several lanes) instead of one or two giant ones; a table of ≥128M rows (8×16M) uses the 16M ceiling. The floor depends on where the row count came from: **1M** for compaction, which reads an exact count from the Delta log, and **8M** for an overwrite, which only has a planner estimate that can be an order of magnitude low. An overwrite of an **existing** table gets a better signal first: the prior version's exact log row count, which raises the estimate when it is larger (never lowers it), so a rebuilt fact table keeps its proper geometry instead of inheriting a bad guess. Appends and `replaceWhere` writes are not sized — they keep the 16M ceiling and let the file roll pick the group — but they do get the rest of the profile. The cap exists because one Parquet row group maps to one column segment in a Direct Lake reader, and Fabric prefers segments in the **8–16M** row band. 16M sits at the top of that band (kept under 2²⁴ so one row group still maps to one segment); arrow-rs buffers a full uncompressed row group per open writer, so this is also the write-memory ceiling. arrow-rs's ~1M default would produce thousands of small segments. Staying under 2²⁴ is a **safe default, not the dominant term**: exceeding it degrades gracefully (a row group simply spans more than one segment), and that penalty is readily outweighed by a good **sort key**, which is the larger lever on read performance (see [Automatic sorting](#automatic-sorting)). Order of priority is sort key first, row-group size second. |
...
What still differs between the two is only the ceiling. A full-table overwrite sizes it from the result (see above); an append leaves it at the 16M maximum and lets the 256 MB file roll pick the actual group, because an increment knows nothing about the table it is landing in. A **`replaceWhere`** write — the microbatch strategy, `INSERT … REPLACE WHERE` — is an increment too, however large: it replaces a *slice*, so it is left unsized for the same reason.
...
Note that this loop grounds the **values** in the table above; it does not ground *how the row count reaching them is obtained*. Changes to the estimate itself — the prior-version log floor, the 8M guess floor — do not move any property here, they only make the existing rule reach the size it always intended. They are still run through the benchmark, because a more accurate estimate does change realized geometry on real tables.
...
- **Measures never hold a dimension slot, but they may follow one.** A measure (`DECIMAL`/`FLOAT`/`DOUBLE`) is an output you aggregate, not a key you filter on, so it can never take one of the four dimension slots. It can, however, earn a **tail slot** *below* the whole dimension key (up to 4 more), where ordering only refines rows *inside* an existing dimension group — correlated or zero-heavy measures collapse into runs there while every dimension a query filters on stays exactly as clustered as it was. A tail slot is kept only if it pays for itself twice over: a worthwhile fraction of the measure's own modelled bytes *and* of the whole table's. *Where it can fail:* the model scores only the column in the slot, so clustering a slot induces on **other** columns is invisible to it — a key whose win comes mostly from that induced effect will be passed over.
  - *A note on how this one is measured.* The question a tail slot asks — how many distinct values does this measure take **inside one dimension group** — is the one question a uniform row sample cannot answer, because once the dimension key is fine there are only a handful of sampled rows left in each group and every measure looks unique. On a real 142M-row fact with ~2.6M `(date, time)` groups, an 8M-row sample leaves ~3 rows per group, and a regional price taking exactly 5 values per interval is indistinguishable from a per-row measure: the tail sees nothing to win and awards no slot, silently missing a ~23% saving. So when groups get dense relative to the sample, this step re-reads **whole groups** — every row of a 1-in-K slice of them — straight from the source, and measures the run fraction there. It is one bounded pass (targeting a couple of million rows regardless of table size), it is only taken when the sample is too thin to decide, and if the source is not reachable the step falls back to sample-only counts and simply keeps its old blind spot.

**Determinism.** The picker is a deterministic function of the profiling sample, and that sample is seeded (`REPEATABLE`), so profiling one table twice returns the same key. It was not always: before the seed was defaulted, two builds of one unchanged model could choose different keys purely on sampling luck — measured at 8.8% of a 592M-row table's size when a different measure won the last tail slot.
