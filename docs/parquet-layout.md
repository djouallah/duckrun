...
Size is not the whole story — VertiPaq also seems to prefer segments of **uniform** size. A hot scan hands out work segment by segment, so equal segments keep the pool balanced, and Microsoft's own storage guidance flags [non-uniform scan load](https://learn.microsoft.com/en-us/fabric/fundamentals/direct-lake-understand-storage) as a hazard: a runt segment still costs its dictionary merge and its scan slot while carrying only a fraction of the rows. duckrun's row groups are uniform within a file by construction — the fixed 6M ceiling — but a row group cannot span files, so the file-size roll cuts every file's **last** group wherever it happens to land. That truncation is why the target file size stays at 256 MB rather than 128 (measurements in the table below): fewer files means fewer truncated tails, until delta-rs learns to align the file roll to a row-group boundary.
...
| **Target file size** | 256 MB | A row group cannot span files, so this byte cap is effectively a segment cap — and every file's **last** row group is truncated wherever the roll lands. That is the reason 256 beats 128 here: halving the file size doubles the file count and with it the number of truncated tail segments (measured on the 142M-row AEMO mart: 4 roll-truncated tails of 25 groups at 128 MB, one a 1.25M-row runt, vs ~2 at 256 MB). A narrow fact reaches a full 6M-row segment well under 256 MB; a wide fact fills 256 MB first, so there the file size — not the row count — sets the segment. It is deliberately well below **1 GB**, which forced whole-file copy-on-write and inflated merge cost on disk; smaller also costs merge *disk* at scale (more files → more DataFusion spill in a whole-table merge). |
