...
What still differs between the two is only the ceiling. A full-table overwrite sizes it from the result (see above); an append leaves it at the 16M maximum and lets the 256 MB file roll pick the actual group, because an increment knows nothing about the table it is landing in.
