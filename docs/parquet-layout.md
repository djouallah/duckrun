...
| **Row group size** | 8M rows | One Parquet row group maps to one column segment in a Direct Lake reader, and Fabric prefers segments in the **8–16M** row band. 8M sits at the low end of that band while bounding write-time memory (arrow-rs buffers a full uncompressed row group per open writer). arrow-rs's ~1M default would produce thousands of small segments. |
| **Dictionary page limit** | 8 MB | Caps how large a column's dictionary grows before the column overflows to **PLAIN**. Mid-cardinality columns (dictionary < 8 MB) stay dictionary-encoded — compact, cheap, and directly remappable by a columnar reader. High-cardinality and unique columns overflow to PLAIN, which is the correct outcome: their dictionary would be nearly as large as the data itself, pure overhead. This bound is also critical for merge memory — a 128 MB limit kept high-cardinality columns dictionary-encoded and caused a merge *reading* the table to materialize ~25 GB of dictionaries; 8 MB keeps the same workload around ~4 GB. The exact crossover point depends on your column cardinalities. |
...
| **Target file size** | 256 MB | A row group cannot span files, so this byte cap is effectively a segment cap — 256 MB allows a wide fact table to reach a full 8M-row segment, one row group per file, producing large uniform segments. It is deliberately well below **1 GB**, which forced whole-file copy-on-write and inflated merge cost on disk. (With the dictionary bounded, file size is no longer a merge-memory lever — 128/256/512 MB merge with similar memory footprints.) |
