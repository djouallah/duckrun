...
- **16M-row row groups** — a row group becomes a segment as-is, so row-group sizing *is* segment sizing; the 8–16M band is a property of the in-memory engine, not of Parquet. One refinement: a **small** full-table write (any overwrite — `CREATE TABLE … AS SELECT`, a dbt `table` model, `--full-refresh`) is written with smaller row groups targeting ~8 groups, so a small table still transcodes on several lanes instead of one or two; results of ≥128M rows keep the 16M profile. (`DUCKRUN_RG_LANES`, default 8. This is computed at the one overwrite seam so the SQL and dbt paths behave identically.)
