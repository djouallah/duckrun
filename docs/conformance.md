...
| `delete+insert` | ✅ | true delete+insert (duplicate-tolerant): delete the matched keys, insert every incoming row, committed as one **fenced overwrite** snapshot-pinned to the version read (delta_rs has no two-commit delete+insert) |
