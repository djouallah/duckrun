...
| `DeltaTable` | `forName`, `forPath`, `convertToDelta`, `merge`, `version`, `history`, `delete`, `update`, `optimize`, `optimize_experimental_sort`, `table_optimize_auto_keys`, `table_optimize_user_keys`, `table_optimize_where_scopes_partitions`, `table_optimize_rejects_query_frame`, `get_rle_scan_count_is_constant`, `optimize_sort_bad_value`, `conn_optimize_shortcut`, `vacuum`, `restoreToVersion` | 19/19 ✅ |
